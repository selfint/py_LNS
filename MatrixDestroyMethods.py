...
class DestroyHeuristic:
    ...
    def generate_subset(self) -> npt.NDArray:
        raise NotImplementedError()


class RandomDestroyHeuristic(DestroyHeuristic):
    def generate_subset(self, initial_subset=[]) -> npt.NDArray:
        # Remove chosen agents from available options
        random_ids = [
            agent_id
            for agent_id in range(1, self.instance.num_agents + 1)
            if agent_id not in initial_subset
        ]
        ...
        subset = (
            initial_subset
            + np.random.choice(random_ids, random_size, replace=False).tolist()
        )
        return subset
...
class ConnectedComponentDestroyHeuristic(DestroyHeuristic):
    def generate_subset(self, complete_random=True):
        ...
        adj_matrix = self.path_table.get_collisions_matrix()
        largest_cc = get_largest_connected_component(adj_matrix)
        if len(largest_cc) > self.subset_size:
            return random_walk_until_neighborhood_is_full(
                adj_matrix, largest_cc, subset_size=self.subset_size
            )
        if len(largest_cc) < self.subset_size:
            # random_dh = RandomDestroyHeuristic(self.instance, self.path_table, self.subset_size)
            return self.fill_cc_with_random_walks(
                largest_cc
            )  # random_dh.generate_subset(initial_subset=largest_cc)
        return largest_cc
    ...
    def fill_cc_with_random_walks(self, connected_component):
        ...
        while len(chosen_ids) < self.subset_size:
            agent_id = self.random_walk_from_random_point_until_collision(
                chosen_ids, connected_component
            )
    ...
    def random_walk_from_random_point_until_collision(
        self, chosen_ids, connected_component
    ):
...
class ArgmaxDestroyHeuristic(DestroyHeuristic):
    def generate_subset(self):
        ...
        for agent in self.instance.agents.values():
            cols += [self.path_table.count_collisions_points_along_existing_path(agent.id, agent.path_id)]
        cols = np.array(cols)
...
class PriorityDestroyHeuristic(DestroyHeuristic):
    def generate_subset(self, argmax_size=0):
        ...
        argmax_ids = []
        if argmax_size > 0:
            argmax_dh = ArgmaxDestroyHeuristic(
                self.instance, self.path_table, argmax_size
            )
            argmax_ids = argmax_dh.generate_subset()
        random_ids = [
            agent_id
            for agent_id in range(1, self.instance.num_agents + 1)
            if agent_id not in argmax_ids
        ]
        subset = (
            argmax_ids
            + np.random.choice(random_ids, random_size, replace=False).tolist()
...
class RandomWeightedDestroyHeuristic(DestroyHeuristic):
    def generate_subset(self, initial_subset=[]):
        ...
        random_ids_mask = np.array(
            [
                0 if agent_id in initial_subset else 1
                for agent_id in range(1, self.instance.num_agents + 1)
            ]
        )
        ...
        random_subset = np.random.choice(
            range(1, self.instance.num_agents + 1),
            random_size,
            replace=False,
            p=prob_weights,
        )
