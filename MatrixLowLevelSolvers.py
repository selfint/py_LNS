...
class NeighborhoodRepair:
    ...
    def destroy_neighborhood(self):
        for agent_id in self.agent_subset:
            ...
            self.path_table.remove_path(agent_id, path_id)
...
class PPNeighborhoodRepair(NeighborhoodRepair):
    ...
    def reroute_agent(self, agent_id):
        ...
        self.path_table.insert_path(agent_id, best_path_id)
...
class RankPPNeighborhoodRepair(NeighborhoodRepair):
    ...
    def calculate_agent_path_costs(self):
        return {
            agent_id: self.path_table.get_agent_collisions_for_paths(
                self.instance.agents[agent_id], self.instance.num_agents
            )
            for agent_id in self.temp_subset
        }
    ...
    def calculate_total_agent_cost(self, path_costs_dict):
        ...
        if self.agent_cost_type == "min":
            res_dict = {
                agent_id: min(path_degrees)
                for agent_id, path_degrees in path_costs_dict.items()
            }
        if self.agent_cost_type == "mean":
            res_dict = {
                agent_id: sum(path_degrees) / len(path_degrees)
                for agent_id, path_degrees in path_costs_dict.items()
            }
