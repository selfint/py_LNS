...
class CBS:
    def __init__(
        ...
        self.n_paths = len(paths[0])
        assert all(len(p) == self.n_paths for p in paths)

        self.cmatrix = torch_parallel_lns.build_cmatrix_fast(paths)
        self.cost_matrix = torch_parallel_lns.build_cost_matrix_from_paths(paths).int()
    ...
    def search(self):
        ...
        # Line 3 of CBS
        initial_node.cost = int(self.compute_solution_cost(initial_node.solution))

        solution_matrix = torch_parallel_lns.solution_cmatrix(
            self.cmatrix, initial_node.solution
        )
        ...
        while self.open:
            ...
            if (
                (len(self.open) & (len(self.open) - 1) == 0)
                and len(self.open) != 0
                and self.verbose
            ):
                print(
                    f"**** popped node with cost {p.cost} from open of size {len(self.open)}"
                )
            ...
            solution_matrix = torch_parallel_lns.solution_cmatrix(
                self.cmatrix, p.solution.clip(0, 1)
            )
            ...
            for agent_id, other_agent in zip(c, c[-1::-1]):
                ...
                a.add_restriction(
                    agent_id,
                    a.get_path_id(agent_id),
                    other_agent,
                    a.get_path_id(other_agent),
                )
    ...
    def compute_collisions_between_path_and_agent(
        self, agent_1_id, agent1_path_id, agent_2_id
    ):
        cmatrix_view = self.cmatrix.view(
            self.n_agents,
            self.n_paths,
            self.n_agents,
            self.n_paths,
...
class CTNode:
    cost: int
    collisions: torch.types.Tensor | int
    col_count: int
    ...
    def __lt__(self, other):
        ...
        return self.col_count < other.col_count or (
            self.col_count == other.col_count and self.cost < other.cost
        )
